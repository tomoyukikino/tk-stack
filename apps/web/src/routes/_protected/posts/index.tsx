...
import { Tooltip, TooltipTrigger, TooltipContent, TooltipArrow, TooltipProvider } from '@repo/ui/components/tooltip'
...
import { postsSearchSchema, postsSearchDefaults, type PostSearchSchema } from '@/validations/posts-link-options'

export const Route = createFileRoute('/_protected/posts/')({
  loader: () => queryClient.ensureQueryData(trpc.posts.all.queryOptions()),
  component: RouteComponent,
  validateSearch: (input: SearchSchemaInput) => v.parse(postsSearchSchema, input),
  search: {
  ...
  return (
    <div className="flex flex-col md:p-4 w-full max-w-6xl mx-auto">
          ...
          <Tooltip delayDuration={0}>
            <TooltipTrigger asChild onClick={e => e.preventDefault()}>
              <Button
                ...
                onClick={() => updateFilters('sortDirection', search.sortDirection === 'asc' ? 'desc' : 'asc')}
              >
                {search.sortDirection === 'asc' ? <ArrowUpIcon /> : <ArrowDownIcon />}
              </Button>
            </TooltipTrigger>
            <TooltipContent
              side="top"
      ...
      <div className="flex gap-x-3 gap-y-3 flex-wrap mt-6">
        {filteredPost?.length
          ? filteredPost.map(p => <PostItem key={p.id} post={p} disabled={isPending} />)
